...
${wifiif} = `uci -q get network.\@device[2].name`;
chomp ${wifiif};
$phy = get_wlan2phy("${wifiif}");

$radio = "radio".substr($phy, -1);
($junk, $chanbw) = &uci_get_named_option("wireless", "$radio", "chanbw");
chomp $chanbw;

if    ($chanbw eq "5")  {$chanbw = 4;}
elsif ($chanbw eq "10") {$chanbw = 2;}
else   			{$chanbw = 1;}
...
foreach(`echo /lin | nc 127.0.0.1 2006 2>>$tmperr`)
{
    ...
    if ( $mac && $fh )
    {
        ...
        if ( -d "/sys/kernel/debug/ieee80211/${phy}/ath9k" )
	    {
            # is the ath9k 802.11n driver
            #802.11b/n
	    ...
	    {
            # must be the ath10k 802.11ac driver
            $mbps = "0";
            $staFound = "";
            foreach(`cat /sys/kernel/debug/ieee80211/${phy}/ath10k/fw_stats`)
            {
                chomp ;
                if(/^[ \t]*Peer MAC address $mac/)
                {
                    $stafound = "yes";
                    next;
                }
                if ( $stafound && /^.*Peer TX rate (\d+)/)
                {
                    $mbps = $1;
                    $mbps /= 1000 ;
                    break;
